...
With 'love nwantiti' we will look at the self-similarity matrix for chroma features, timbre features
...
### Mode and Loudness
The modes seem quite evenly divided. although in pop we see that a lot of songs with low energy are in major, where in kizomba that was in minor. The loudness in bachata songs is the highest as there the dors are bigger. In the pop playlist the 
...
```{r Sad Girlz Luv Money original chroma, echo=FALSE}

  library(compmus)
  compmus_long_distance(
  sadgirlzoriginal %>% mutate(pitches = map(pitches, compmus_normalise, "chebyshev")),
  sadgirlzoriginal %>% mutate(pitches = map(pitches, compmus_normalise, "chebyshev")),
  feature = pitches,
  method = "manhattan"
) %>%
  ...
  geom_tile() +
  coord_equal() +
  labs(x = "sadgirlzoriginal", y = "sadgirloriginal") +
  theme_minimal() +
  scale_fill_viridis_c(guide = NULL)
  
```

```{r Sad Girlz Luv Money, echo=FALSE}
  ...
  geom_tile() +
  coord_equal() +
  labs(x = "sadgirlzoriginal", y = "sadgirlzkizomba") +
  theme_minimal() +
  scale_fill_viridis_c(guide = NULL)
